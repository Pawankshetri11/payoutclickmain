...
const ReferralSystem = () => {
  ...
  const fetchReferralData = async () => {
    try {
      setLoading(true);
      console.log('🔄 Fetching referral data for admin panel...');
      
      // Get all referral relationships from referrals table
      const { data: referralRows, error: refError } = await (supabase as any)
        .from('referrals')
        .select('*')
        .order('created_at', { ascending: false });

      console.log('📊 Referrals table data:', referralRows);
      ...
      (referralRows || []).forEach((r: any) => {
        ...
        if (r.referred_id) allUserIds.add(r.referred_id);
      });
      ...
      const referralRelationships = (allProfiles || []).map((profile: any) => {
        const referredBy = profile.referred_by;
        const referrerProfile = profileMap.get(referredBy);
        const refRow = (referralRows || []).find((r: any) => 
          r.referrer_id === referredBy && r.referred_id === profile.user_id
        );
        ...
        return {
          ...
          commissionEarned: refRow?.total_commission_earned || 0,
          withdrawals: 0 // TODO: Count actual withdrawals
        };
      });
      ...
      const commissionHistory = (commissionTransactions || []).map((tx: any) => {
        const referrerProfile = profileMap.get(tx.user_id);
        return {
          id: tx.id,
          referrer: referrerProfile?.name || 'Unknown',
          referrerEmail: referrerProfile?.email || '',
          referee: tx.description?.split('from')[1]?.trim().split('(')[0]?.trim() || 'Referred User',
          withdrawalAmount: Math.round(tx.amount / 0.10),
          commissionAmount: tx.amount,
          companyFee: tx.amount,
      ...
      // Calculate stats
      const totalCommissions = commissionHistory.reduce((sum: number, c: any) => sum + c.commissionAmount, 0);
      const companyRevenue = totalCommissions;
