...
const ReferralSystem = () => {
  ...
  const fetchReferralData = async () => {
    try {
      ...
      const { data: commissionTransactions, error: commError } = await supabase
        .from('transactions')
        .select('id, user_id, amount, description, type, status, category, created_at')
        .eq('type', 'earning')
      ...
      const userIds = [...new Set((commissionTransactions || []).map((tx: any) => tx.user_id))];
      ...
      const commissionHistory = (commissionTransactions || []).map((tx: any) => {
        ...
        const refereeName = match ? match[1] : 'Referred User';
        const refereeId = match ? match[2] : '';
        
        return {
          id: tx.id,
          referrer: profile?.name || 'Unknown',
          referrerEmail: profile?.email || '',
          referee: refereeName,
          refereeId: refereeId,
          withdrawalAmount: tx.amount / 0.10, // Commission is 10%, so withdrawal = commission / 0.10
          commissionAmount: tx.amount,
          companyFee: tx.amount, // Company also gets 10%
      ...
      const companyRevenue = totalCommissions; // Same as commissions since both are 10%

      // Get unique referrers
      const uniqueReferrers = new Set(commissionHistory.map((c: any) => c.referrer));

      setStats({
        totalReferrals: commissionHistory.length,
        activeReferrals: commissionHistory.filter((c: any) => c.status === 'completed').length,
        totalCommissions,
        companyRevenue,
      });
    } catch (error: any) {
