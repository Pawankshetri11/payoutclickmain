...
const Settings = () => {
  ...
  const [globalLockMinutes, setGlobalLockMinutes] = useState<number>(60);
...
useEffect(() => {
  if (systemSettings && Object.keys(systemSettings).length > 0) {
    ...
    setGlobalLockMinutes(getSetting('global_review_lock_minutes', 60));
  }
}, [systemSettings]);
...
const handleSaveAllSettings = async () => {
  try {
    await Promise.all([
      ...
      updateSetting('global_review_lock_minutes', Number(globalLockMinutes)),
    ]);
        ...
        <CardContent>
          <Tabs defaultValue="general" className="space-y-6">
            <TabsList className="grid w-full grid-cols-7">
              ...
              <TabsTrigger value="dates">Payment Dates</TabsTrigger>
            ...
            <TabsContent value="general" className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="global-lock-minutes">Global Review Lock (minutes)</Label>
                    <Input 
                      id="global-lock-minutes" 
                      type="number"
                      min="0"
                      value={globalLockMinutes}
                      onChange={(e) => setGlobalLockMinutes(Number(e.target.value) || 0)}
                    />
                    <p className="text-xs text-muted-foreground">
                      Time users must wait before starting any other review profile.
                    </p>
                  </div>
                </div>
