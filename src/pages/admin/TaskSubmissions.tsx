...
const TaskSubmissions = () => {
  ...
  const handleApprove = async (taskId: string) => {
    ...
    try {
      ...
      // Credit earnings to user balance and update counters
      const { data: profile, error: profileFetchError } = await supabase
        .from('profiles')
        .select('balance, total_earnings, completed_tasks')
      ...
      const newTotalEarnings = ((profile as any).total_earnings || 0) + task.amount;
      const newCompletedTasks = ((profile as any).completed_tasks || 0) + 1;
