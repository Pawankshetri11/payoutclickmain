        ...
        <CardContent>
          <Tabs defaultValue="pending">
            ...
            {['pending', 'approved', 'rejected'].map((status) => {
              const statusTasks = status === 'pending' ? pendingTasks : 
                      ...
                      {loading ? (
                        ...
                        statusTasks.map((task) => (
                          <TableRow key={task.id} className="hover:bg-accent/50">
                            <TableCell>
                              <div>
                                <div className="font-medium">{task.profiles?.name || 'Unknown'}</div>
        ...
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          {selectedTask && (
            <div className="space-y-4">
              ...
              {selectedTask.submission_code && (
                <div>
                  <Label>Submitted Code</Label>
                  <p className="text-sm font-medium p-2 bg-muted rounded">{selectedTask.submission_code}</p>
                </div>
              )}

              {selectedTask.submission_image && (
                <div>
                  <Label>Submitted Image</Label>
                  <img 
                    src={selectedTask.submission_image} 
                    alt="Task submission" 
