...
const TaskSubmissions = () => {
  ...
  const fetchTasks = async () => {
    try {
      ...
      if (error) {
        // If relationship missing, fallback to query without profiles join
        const relationMissing = (error as any)?.message?.includes('relationship between') && (error as any)?.code === 'PGRST200';
