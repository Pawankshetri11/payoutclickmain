...
const Users = () => {
  ...
  const handleLoginAsUser = async (userId: string, userEmail: string) => {
    try {
      ...
      console.log('🔐 Calling impersonate function with refreshed session');
      ...
      if (data?.url) {
        console.log('✅ Got magic link, redirecting...');
        toast.success('Logging in as user...');
        // Replace current location to avoid back button issues
        window.location.replace(data.url);
      } else {
        console.error('No URL in response:', data);
        toast.error('No login URL received. Please try again.');
      }
    } catch (err: any) {
      console.error('Impersonation error:', err);
      toast.error(err.message || 'Failed to login as user. Please refresh and try again.');
    }
  };
