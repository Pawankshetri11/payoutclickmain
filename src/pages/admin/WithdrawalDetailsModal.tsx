...
export function WithdrawalDetailsModal({ 
  ...
  const fetchWithdrawalDetails = async () => {
    ...
    try {
      ...
      // Fetch screenshot if exists
      const { data: wdData } = await (supabase as any)
        .from('withdrawals')
        .select('screenshot_url')
        .eq('id', withdrawal.id)
        .single();
      
      if (wdData?.screenshot_url) {
        setScreenshot(wdData.screenshot_url);
      }
    } catch (error) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-6 py-4">
          ...
          {/* Payment Screenshot (if uploaded by admin) */}
          {screenshot && (
            <div>
              <h3 className="font-semibold text-foreground mb-3 flex items-center gap-2">
