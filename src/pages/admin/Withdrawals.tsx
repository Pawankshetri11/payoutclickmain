...
const Withdrawals = () => {
  ...
  const handleInstantWithdrawal = async (userId: string, amount: number) => {
    try {
      ...
      const { data: existingPending, error: pendingErr } = await (supabase as any)
        .from('withdrawals')
        .select('id, status')
        .eq('user_id', userId)
        .in('status', ['pending']);
      ...
      if (pm) {
        if (pm.method_type === 'bank') {
          accountDetails.bank_name = pm.bank_name;
          accountDetails.account_holder = pm.account_holder;
          accountDetails.account_number = pm.account_number;
          accountDetails.routing_number = pm.routing_number;
        } else if (pm.method_type === 'upi') {
          accountDetails.upi_id = pm.upi_id;
        } else if (pm.method_type === 'paypal') {
          accountDetails.paypal_email = pm.paypal_email;
        } else if (pm.method_type === 'crypto') {
          accountDetails.crypto_wallet_address = pm.crypto_wallet_address;
          accountDetails.crypto_network = pm.crypto_network;
        }
      }
      ...
      const { error: wdErr } = await (supabase as any)
        ...
        .insert({
          ...
          status: 'pending',
          method: pm?.method_type || 'bank',
          payment_method: pm?.method_type || 'bank',
          account_details: Object.keys(accountDetails).length > 0 ? accountDetails : { note: 'No payment method configured' },
        });
  ...
  const handleBulkWithdrawal = async () => {
    ...
    try {
      for (const user of users) {
        // Skip if balance < 100 or KYC not verified
        const amount = Number(user.balance || 0);
        console.log(`User ${user.email}: balance=${amount}, kyc=${user.kyc_status}`);
        
        if (amount < 100 || user.kyc_status !== 'verified') {
          console.log(`Skipping user ${user.email}: insufficient balance or KYC not verified`);
          skipped++;
          continue;
        }
        ...
        try {
          ...
          if (pendingErr) {
            console.error(`Error checking pending withdrawal for ${user.email}:`, pendingErr);
            skipped++;
            continue;
          }
          
          if (existingPending && existingPending.length > 0) {
            console.log(`Skipping user ${user.email}: already has pending withdrawal`);
            skipped++;
            continue;
          }
          ...
          if (wdErr) {
            console.error(`Failed for user ${user.email}:`, wdErr);
            skipped++;
            continue;
          }

          console.log(`Created withdrawal for user ${user.email}`);
          created++;
        } catch (err) {
      ...
      console.log(`Bulk withdrawal complete: created=${created}, skipped=${skipped}`);

      if (created > 0) {
        toast.success(`Created ${created} withdrawal requests. Skipped ${skipped}.`);
        refetch();
      } else {
        toast.error(`No withdrawal requests created. Skipped ${skipped} users.`);
      }
    } catch (error: any) {
