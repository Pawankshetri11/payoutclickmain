...
export default function Earnings() {
  ...
  // Realtime subscription to auto-refresh on new transactions/withdrawals
  useEffect(() => {
    if (!user) return;
  ...
  const mappedTransactions = userTransactions
    ...
    .map(tr => {
      ...
      const isReferralCommission = typeLower === 'earning' && descLower.includes('referral commission');
  ...
  // Deduplicate withdrawals - prefer transactions table over withdrawals table
  const withdrawalTxnIds = new Set(
    mappedTransactions
      .filter(t => t.category === 'withdrawal')
      .map(t => t.id)
  );
  
  const deduplicatedManualWithdrawals = manualWithdrawalTransactions.filter(w => {
    // Check if this withdrawal already exists in transactions table
    const existsInTransactions = mappedTransactions.some(t => 
      t.category === 'withdrawal' && 
      t.amount === w.amount && 
      Math.abs(new Date(t.date).getTime() - new Date(w.date).getTime()) < 120000 // within 2 minutes
    );
    
    // Also check if the withdrawal ID already exists in transactions table
    const idExistsInTransactions = mappedTransactions.some(t => t.id === w.id);
    
    return !existsInTransactions && !idExistsInTransactions;
  });

  const allTransactionsUnique = [...taskTransactions, ...mappedTransactions, ...deduplicatedManualWithdrawals]
    .sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());

  // Filter transactions by category
  const transactions = allTransactionsUnique.slice(0, 10);
  
  // Earnings tab: task earnings, referral commission, admin credit/debit
  const earningTransactions = allTransactionsUnique.filter(t => 
    t.category === 'earning' || 
    t.category === 'task' || 
    t.category === 'general' || 
    t.category === 'referral' ||
    t.category === 'adjustment' // Include admin credit/debit
  );
  
  // Withdrawals tab: ONLY actual withdrawals (NOT admin operations)
  const withdrawalTransactions = allTransactionsUnique.filter(t => 
    t.category === 'withdrawal' && 
    !t.title.toLowerCase().includes('admin')
  );
  
  // Referrals tab: only referral commission
  const referralTransactions = allTransactionsUnique.filter(t => 
    t.category === 'referral'
  );
