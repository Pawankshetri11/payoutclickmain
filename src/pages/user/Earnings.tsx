...
export default function Earnings() {
  ...
  const mappedTransactions = userTransactions.map(tr => {
    ...
    const isWithdrawal = typeLower === 'withdrawal' && !descLower.includes('admin');
    const isTaskEarning = typeLower === 'earning' && !isReferralCommission && !isAdminCredit && !descLower.includes('task');
    ...
    if (isReferralCommission) {
      ...
      displayType = 'withdrawal';
      isNegative = true;
    } else if (isTaskEarning) {
      // Skip task earnings from transactions table as they come from tasks
      return null;
    }
    ...
    return {
      ...
      date: tr.created_at,
      status: tr.status,
      category: isReferralCommission ? 'referral' : (isAdminCredit || isAdminDebit ? 'adjustment' : (isWithdrawal ? 'withdrawal' : 'earning')),
      icon,
      isNegative
    };
  ...
  const transactions = [...taskTransactions, ...mappedTransactions, ...manualWithdrawalTransactions]
