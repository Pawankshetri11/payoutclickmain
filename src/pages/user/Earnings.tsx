...
export default function Earnings() {
  ...
  // Build transactions from tasks (earnings)
  ...
  // Include admin adjustments and withdrawals from transactions table
  const mappedUserTransactions = userTransactions
    .filter(tr => tr.type !== 'earning')
    .map(tr => {
      const typeLower = (tr.type || '').toLowerCase();
      const isCredit = ['admin_credit', 'credit', 'adjust_credit'].includes(typeLower) || tr.amount > 0;
      const isDebit = ['admin_debit', 'debit', 'adjust_debit'].includes(typeLower) || tr.amount < 0;
      const normalizedType = typeLower === 'withdrawal' ? 'withdrawal' : (isCredit || isDebit ? 'admin_adjustment' : (tr.type || 'transaction'));
      return {
        id: tr.id,
        type: normalizedType,
        title: normalizedType === 'withdrawal' ? 'Withdrawal' : isCredit ? 'Admin Credit' : isDebit ? 'Admin Debit' : (tr.description || 'Transaction'),
        amount: tr.amount,
        date: tr.created_at,
        status: tr.status,
        category: 'adjustment',
        icon: normalizedType === 'withdrawal' ? Wallet : (isCredit ? ArrowUpRight : ArrowDownRight)
      };
    });

  const transactions = [...taskTransactions, ...mappedUserTransactions]
                  ...
                  transactions.filter(t => t.type === 'withdrawal').map((transaction) => (
                    <div key={transaction.id} className="flex items-center justify-between p-4 rounded-lg border border-border/50 bg-background/50">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="font-medium text-sm text-foreground">Withdrawal</p>
