    ...
    const load = async () => {
      try {
        const [txRes, wdRes] = await Promise.all([
          supabase
            .from('transactions')
            .select('id, type, amount, status, created_at, description, category')
            .eq('user_id', user.id)
  ...
  const mappedTransactions = userTransactions
    ...
    .map(tr => {
      ...
      if (isReferralCommission) {
        ...
        // Admin debit is a transaction, not withdrawal
        title = 'Admin Debit';
        icon = Wallet;
      ...
      return {
        ...
        date: tr.created_at,
        status: tr.status,
        category: isReferralCommission ? 'referral' : (isAdminCredit ? 'adjustment' : (isAdminDebit ? 'debit' : (isWithdrawal ? 'withdrawal' : 'earning'))),
        icon,
        isNegative,
