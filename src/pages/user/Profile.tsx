...
import { Camera, Mail, Phone, Calendar, Edit, Shield, Award, TrendingUp } from "lucide-react";
...
export default function Profile() {
  ...
  // Force refetch on mount to get latest completed_tasks count
  useEffect(() => {
    const fetchLatestProfile = async () => {
      await refetch();
      console.log('Profile refetched on mount:', profile);
    };
    fetchLatestProfile();
  }, []);
  ...
  console.log('Completed tasks value:', profile?.completed_tasks);

  const stats = [
    { label: "Tasks Completed", value: profile?.completed_tasks || 0, icon: Award, color: "text-success" },
    { label: "Total Earnings", value: `₹${profile?.total_earnings || 0}`, icon: TrendingUp, color: "text-primary" },
    { label: "Current Balance", value: `₹${profile?.balance || 0}`, icon: Shield, color: "text-accent" },
    { label: "Member Since", value: profile?.created_at ? new Date(profile.created_at).toLocaleDateString() : "N/A", icon: Calendar, color: "text-muted-foreground" }
  ];
