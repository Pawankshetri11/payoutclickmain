...
export default function ReviewTask() {
  ...
  // Auto-restore in-progress profile for THIS user only
  useEffect(() => {
    // Auto-restore ONLY when not on the profile selection screen
    if (taskStage !== 'profile-selection' && inProgressProfile && !selectedProfile && profiles.length > 0) {
      const profile = profiles.find(p => p.id === inProgressProfile && p.status === 'locked_by_you');
      if (profile) {
        setSelectedProfile(profile);
        setTaskStage('review-content');
      }
    }
  }, [inProgressProfile, profiles, selectedProfile, taskStage]);

  const handleSelectProfile = async (profileId: string) => {
    try {
      const profile = profiles.find(p => p.id === profileId);
      
      // If profile is already locked by user, just continue (don't lock again)
      if (profile?.status === 'locked_by_you') {
        setSelectedProfile(profile);
        setTaskStage('review-content');
        return;
      }
      
      // Otherwise, select and lock the profile
      const selectedProf = await selectProfile(profileId);
      if (selectedProf) {
  ...
  useEffect(() => {
    const ensureSelectedReview = async () => {
      if (!user || !selectedProfile) return;

      // Get persisted selected review for this user+profile
      ...
      let reviewId = submission?.review_id;

      // Backfill if missing: pick first active review and persist it
      if (!reviewId && reviews.length > 0) {
        const activeReviews = reviews.filter((r: any) => r.status === 'active');
      ...
      if (reviewId) {
        const chosen = reviews.find((r: any) => r.id === reviewId);
        if (chosen) setSelectedReview(chosen);
      }
    };

    ensureSelectedReview();
  }, [user, selectedProfile, reviews]);
              ...
              return (
                ...
                >
                  ...
                  <CardContent className="space-y-3 pt-0">
                    {profile.unlocks_at && profile.status === 'on_cooldown' && (
                    ...
                    {profile.status === 'locked_by_others' && (
                      <p className="text-xs text-amber-600 dark:text-amber-400 text-center">
                        {profile.blockedByOwnInProgress
                          ? '🔒 Complete your current review first'
                          : (!globalCountdown.isExpired 
