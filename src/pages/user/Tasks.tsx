...
export default function Tasks() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // Fetch user's submission counts per job
      ...
      (submissionCounts || []).forEach((sub: any) => {
        const count = submissionMap.get(sub.job_id) || 0;
        submissionMap.set(sub.job_id, count + 1);
      });
      ...
      // Filter based on submission limit per user and remaining vacancy
      const transformedTasks = (jobs || [])
        .filter(job => {
          const userSubmissions = submissionMap.get(job.id) || 0;
          const submissionLimit = (job as any).submission_limit_per_user || 1;
          const hasVacancy = job.vacancy > job.completed;
          const canSubmitMore = userSubmissions < submissionLimit;
          return hasVacancy && canSubmitMore;
        })
        .map(job => ({
          id: job.id,
          title: job.title,
  ...
  const handleStartTask = (task: any) => {
    if (task.type === 'review') {
      navigate(`/user/jobs/${task.id}/review`);
    } else {
