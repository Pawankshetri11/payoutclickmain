    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-6 py-4">
          {/* Transaction Summary */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-4 bg-muted/20 rounded-lg">
              <div className="space-y-2">
                ...
                {/* Show payment transaction ID if available */}
                {transaction.transaction_id && (
                  <div className="flex items-center gap-2 text-sm">
                    <FileText className="h-4 w-4 text-success" />
                    <span className="text-muted-foreground">Payment Txn ID:</span>
                    <span className="font-mono text-xs text-success font-medium">{transaction.transaction_id}</span>
                  </div>
                )}
          ...
          {/* Fee breakdown for withdrawals (not for admin transactions) */}
          {transaction.type === 'withdrawal' && 
           transaction.category === 'withdrawal' && 
           !transaction.title.toLowerCase().includes('admin') && (
