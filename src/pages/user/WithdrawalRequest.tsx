...
export default function WithdrawalRequest() {
  ...
  const fetchUserData = async () => {
    ...
    try {
      ...
      // Check if user has a referrer using type assertion
      const profileWithReferrer = data as any;
      setReferrerId(profileWithReferrer?.referred_by || null);
  ...
  const fetchUserWithdrawals = async () => {
    ...
    try {
      ...
      const combined = [...txList, ...wdList].sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
      setUserWithdrawals(combined);
    } catch (error: any) {
  ...
  const calculateFees = (amount: number, referrerId?: string) => {
    // 20% total deduction
    const companyFee = amount * 0.10; // 10% company platform fee
    const referralFee = referrerId ? amount * 0.10 : amount * 0.10; // 10% referral or extra 10% to company
    
    const totalFee = companyFee + referralFee;
  ...
  const handleSubmitWithdrawal = async () => {
    ...
    try {
      // Calculate fees with referrer
      const fees = calculateFees(amount, referrerId || undefined);
      ...
      // Referral commission will be processed on approval to avoid double-credit
      // if (referrerId && fees.referralFee > 0) {
      //   await processReferralCommission(amount, referrerId, user.id);
      // }
  ...
  const fees = amount > 0 ? calculateFees(amount, referrerId || undefined) : null;
              ...
              {fees && amount > 0 && (
                <div className="p-4 bg-muted/20 rounded-lg border border-border/50">
                  ...
                  <div className="space-y-2 text-sm">
                    ...
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">{fees.hasReferrer ? 'Referral Commission (10%)' : 'Additional Company Fee (10%)'}:</span>
                      <span className="text-destructive">-₹{fees.referralFee?.toFixed(2)}</span>
                    </div>
                    <div className="border-t border-border pt-2">
                      ...
                      <div className="text-xs text-muted-foreground mt-1">
                        Total deduction: 20% (₹{fees.totalFee?.toFixed(2)})
                      </div>
                    </div>
                  </div>
                </div>
